...
def main():
    ...
    if with_detector:
        model.compile(
            ...
            metrics={'out': ['accuracy', dice_coef],
                     'aux_output': []},
    ...
    if action == 'validation_segmentation':
        ...
        test_data = load_dict_from_file(
            '../augmentation/test_segmentation_log.json')
        ...
        train_data = load_dict_from_file(
            '../augmentation/train_segmentation_log.json')
    ...
    if action == 'speed_benchmark':
        ...
        log_file = path.splitext(
            weights_path)[0] + '_validation_segmentation.txt'
        with open(log_file, 'w') as f:
            f.write('Validation, run: {}, weights: {}, '.format(
                run_date, weights_path))
            f.write('loss: {}, accuracy: {}, dice: {}, precision: {}, recall: {}\n'.format(
        ...
        with open(log_file, 'w') as f:
            f.write('Test, run: {}, weights: {}, '.format(
                run_date, weights_path))
            f.write('loss: {}, accuracy: {}, dice: {}\n'.format(
    ...
    elif action == 'train_segmentation' or action == 'train_detector' or \
            action == 'train_all':
