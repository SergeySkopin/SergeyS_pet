...
ANNOTATIONS_FILE= 'ESC-50-master/meta/esc50.csv'
AUDIO_DIR= 'ESC-50-master/audio'

def train(model, train_loader, valid_loader, loss_fn, optimiser, scaler, device, epochs):
...
def run_train(cfg: DictConfig):
    esc_data = ESC50('ESC-50-master/meta/esc50.csv',
                            AUDIO_DIR,
                            get_mel(cfg),
                            cfg.SAMPLE_RATE,
                            cfg.NUM_SAMPLES,
    ...
    train_dataset, valid_dataset = torch.utils.data.random_split(esc_data, [cfg.TRAIN_SIZE, cfg.VALID_SIZE])
    print(f"There are {len(train_dataset)} samples in the dataset.")
    train_dataloader, valid_dataloader = create_data_loader(train_dataset, valid_dataset, cfg.BATCH_SIZE)
    ...
    # train model
    train(cnn, train_dataloader, valid_dataloader, loss_fn, optimiser, scaler, DEVICE, cfg.EPOCHS)
    # save model
